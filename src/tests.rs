...
fn serves_tls_requests_with_cert_from_cache_directory() {
  ...
  let test_agora_download_staging_cert = "
    ...
    iVANK2NpAdYCnlW3zTLO2p2IjEqhRANCAAR0s0wc9Am9HPsOtX81Fhz+yl5TKPCv
    UuD7ECOSsHPnsw31FTs5XqvRvXEEItYZEnoalFQzsGA+6jej63gZgUQx
    -----END PRIVATE KEY-----
...
fn creates_cert_cache_directory_if_it_doesnt_exist() {
  // fixme: library cretes directory itself
  test_with_arguments(
    &["--acme-cache-directory", "cache-directory"],
    |context| async move {
      assert!(context.working_directory().join("cache-directory").is_dir());
    },
  );
}
